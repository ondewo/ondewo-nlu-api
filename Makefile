...
release_all_clients: ## Releases all NLU Clients
	@rm -f .already_released_marker; make release_python_client || { [ -f .already_released_marker ] && echo "SKIP: Python client already released ${ONDEWO_NLU_API_VERSION}" || { echo "FAILED: Python client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_angular_client || { [ -f .already_released_marker ] && echo "SKIP: Angular client already released ${ONDEWO_NLU_API_VERSION}" || { echo "FAILED: Angular client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_nodejs_client || { [ -f .already_released_marker ] && echo "SKIP: Nodejs client already released ${ONDEWO_NLU_API_VERSION}" || { echo "FAILED: Nodejs client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_typescript_client || { [ -f .already_released_marker ] && echo "SKIP: Typescript client already released ${ONDEWO_NLU_API_VERSION}" || { echo "FAILED: Typescript client did not release - aborting"; exit 1; }; }
	@rm -f .already_released_marker; make release_js_client || { [ -f .already_released_marker ] && echo "SKIP: JS client already released ${ONDEWO_NLU_API_VERSION}" || { echo "FAILED: JS client did not release - aborting"; exit 1; }; }
